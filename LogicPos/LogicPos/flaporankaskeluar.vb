...
Public Class flaporankaskeluar
    Public namaform As String = "laporan-kas_keluar"
    ...
    Private Sub flaporankaskeluar_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Call historysave("Membuka Laporan Kas Keluar", "N/A", namaform)
    End Sub
    Sub grid()
        GridColumn1.Caption = "id kas keluar"
    ...
    Sub tabel()
        ...
        If Format(DateTimePicker1.Value, "yyyy-MM-dd").Equals(Format(DateTimePicker2.Value, "yyyy-MM-dd")) Then
            sql = "SELECT tb_kas_keluar.id, tb_kas.kode_kas, tb_user.kode_user, tb_kas.jenis_kas, tb_kas_keluar.tanggal, saldo_kas, tb_kas_keluar.keterangan_kas FROM tb_kas_keluar JOIN tb_kas ON tb_kas.id = tb_kas_keluar.kas_id JOIN tb_user ON tb_user.id = tb_kas_keluar.user_id WHERE DATE(tanggal) = '" & Format(DateTimePicker1.Value, "yyyy-MM-dd") & "' ORDER BY tb_kas_keluar.id ASC"
        Else
            sql = "SELECT tb_kas_keluar.id, tb_kas.kode_kas, tb_user.kode_user, tb_kas.jenis_kas, tb_kas_keluar.tanggal, saldo_kas, tb_kas_keluar.keterangan_kas FROM tb_kas_keluar JOIN tb_kas ON tb_kas.id = tb_kas_keluar.kas_id JOIN tb_user ON tb_user.id = tb_kas_keluar.user_id WHERE DATE(tanggal) BETWEEN '" & Format(DateTimePicker1.Value, "yyyy-MM-dd") & "' AND '" & Format(DateTimePicker2.Value, "yyyy-MM-dd") & "' ORDER BY tb_kas_keluar.id ASC"
        End If
    ...
    Private Sub btntabel_Click(sender As Object, e As EventArgs) Handles btntabel.Click
        Call tabel()
        Call historysave("Merefresh Laporan Kas Keluar", "N/A", namaform)
    End Sub
    Sub ExportToExcel()
    ...
    Private Sub btnexcel_Click(sender As Object, e As EventArgs) Handles btnexcel.Click
        If exportstatus.Equals(True) Then
            If GridView1.DataRowCount > 0 Then
                ExportToExcel()
                Call historysave("Mengexport Laporan Kas Keluar", "N/A", namaform)
            Else
    ...
    Private Sub btnrekap_Click_1(sender As Object, e As EventArgs) Handles btnrekap.Click
        If printstatus.Equals(True) Then
            ...
            If Format(DateTimePicker1.Value, "yyyy-MM-dd").Equals(Format(DateTimePicker2.Value, "yyyy-MM-dd")) Then
                sql = "SELECT * FROM tb_kas_keluar WHERE DATE(tanggal) = '" & Format(DateTimePicker1.Value, "yyyy-MM-dd") & "'"
            Else
                sql = "SELECT * FROM tb_kas_keluar WHERE DATE(tanggal) BETWEEN '" & Format(DateTimePicker1.Value, "yyyy-MM-dd") & "' AND '" & Format(DateTimePicker2.Value, "yyyy-MM-dd") & "'"
            End If
            ...
            If dr.HasRows Then
                ...
                flapkaskeluar.CrystalReportViewer1.ReportSource = rptrekap
                flapkaskeluar.ShowDialog()
                flapkaskeluar.WindowState = FormWindowState.Maximized

                Call historysave("Merekap Laporan Kas Keluar", "N/A", namaform)
            Else
