...
Public Class flunaspiutang
    ...
    Function currentnumber()
        Call koneksi()
        sql = "SELECT id FROM tb_pelunasan_piutang ORDER BY id DESC LIMIT 1;"
        Dim pesan As String = ""
        Try
    ...
    Private Sub prevnumber(previousnumber As String)
        Call koneksi()
        sql = "SELECT id FROM tb_pelunasan_piutang WHERE date_created < (SELECT date_created FROM tb_pelunasan_piutang WHERE id = '" & previousnumber & "') ORDER BY date_created DESC LIMIT 1"
        Dim pesan As String = ""
        Try
            cmmd = New OdbcCommand(sql, cnn)
            dr = cmmd.ExecuteReader
    ...
    Private Sub nextnumber(nextingnumber As String)
        Call koneksi()
        sql = "SELECT id FROM tb_pelunasan_piutang WHERE date_created > (SELECT date_created FROM tb_pelunasan_piutang WHERE id = '" & nextingnumber & "') ORDER BY date_created ASC LIMIT 1"
        Dim pesan As String = ""
        Try
            cmmd = New OdbcCommand(sql, cnn)
            dr = cmmd.ExecuteReader
    ...
    Sub inisialisasi(nomorkode As Integer)
        ...
        If nomorkode > 0 Then
            Call koneksi()
            sql = "SELECT * FROM tb_pelunasan_piutang WHERE id = '" & nomorkode & "'"
            cmmd = New OdbcCommand(sql, cnn)
            dr = cmmd.ExecuteReader
            dr.Read()
            If dr.HasRows Then
                ...
                viewkodecustomer = dr("pelanggan_id")
                ...
                cmbpelanggan.SelectedValue = viewkodecustomer
    ...
    Sub tabel_utama()
        tabel = New DataTable
        With tabel
            .Columns.Add("penjualan_id")
            .Columns.Add("pelanggan_id")
    ...
    Sub tabel_lunas()
        Call gridlunas()
        kode = Me.GridView1.GetFocusedRowCellValue("penjualan_id")
        ...
        Call koneksi()
        sql = "SELECT tb_pelunasan_piutang_detail.pelunasan_piutang_id, tb_pelunasan_piutang.tanggal_transaksi, tb_pelunasan_piutang_detail.terima_piutang FROM tb_pelunasan_piutang_detail JOIN tb_pelunasan_piutang ON tb_pelunasan_piutang.id = tb_pelunasan_piutang_detail.pelunasan_piutang_id WHERE tb_pelunasan_piutang_detail.penjualan_id ='" & kode & "'"
        cmmd = New OdbcCommand(sql, cnn)
        dr = cmmd.ExecuteReader()
        While dr.Read
            tabellunas.Rows.Add(dr("pelunasan_piutang_id"), dr("tanggal_transaksi"), dr("terima_piutang"))
        End While
    ...
    Sub previewpelunasan(lihat As String)
        ...
        dr = cmmd.ExecuteReader()
        While dr.Read
            tabel.Rows.Add(dr("penjualan_id"), dr("pelanggan_id"), dr("tanggal_penjualan"), dr("tanggal_jatuhtempo"), Val(dr("total_penjualan")), Val(dr("bayar_piutang")), Val(dr("sisa_piutang")), Val(dr("terima_piutang")))
            tabelsementara.Rows.Add(dr("penjualan_id"), dr("pelanggan_id"), dr("tanggal_penjualan"), dr("tanggal_jatuhtempo"), Val(dr("total_penjualan")), Val(dr("bayar_piutang")), Val(dr("sisa_piutang")), Val(dr("terima_piutang")))
        End While
    ...
    Sub simpan()
        ...
        For i As Integer = 0 To GridView1.RowCount - 1
            'cek ke penjualan
            Call koneksi()
            sql = "SELECT * FROM tb_penjualan WHERE id = '" & GridView1.GetRowCellValue(i, "penjualan_id") & "'"
            cmmd = New OdbcCommand(sql, cnn)
            dr = cmmd.ExecuteReader
            ...
            Else
                MsgBox("id penjualan : " + GridView1.GetRowCellValue(i, "penjualan_id") & " tidak ditemukan !")
                Exit Sub
            End If

            'IFNULL(SUM(kredit_kas), 0)
            Call koneksi()
            sql = "SELECT IFNULL(SUM(terima_piutang), 0) AS terima_piutang FROM tb_pelunasan_piutang_detail WHERE penjualan_id = '" & GridView1.GetRowCellValue(i, "penjualan_id") & "' AND pelanggan_id ='" & idpelanggan & "'"
            cmmd = New OdbcCommand(sql, cnn)
            dr = cmmd.ExecuteReader
            If dr.HasRows Then
            ...
            If sisajual(i) < Val(GridView1.GetRowCellValue(i, "terima_piutang")) Then
                MsgBox("Kelebihan Bayar pada nota " + GridView1.GetRowCellValue(i, "penjualan_id"))
                Exit Sub
            End If
        Next
        ...
        Try
            Call koneksi()
            sql = "INSERT INTO tb_pelunasan_piutang (user_id, pelanggan_id, kas_id, tanggal_transaksi, jenis_kas, bayar_lunas, no_bukti, keterangan_lunas, void_lunas, print_lunas, posted_lunas, created_by, updated_by, date_created, last_updated) VALUES ('" & iduser & "', '" & idpelanggan & "', '" & idkas & "','" & Format(dtpelunasan.Value, "yyyy-MM-dd HH:mm:ss") & "','" & cmbbayar.Text & "', '" & totalbayar & "','" & txtbukti.Text & "','" & txtketerangan.Text & "','" & 0 & "','" & 0 & "','" & 1 & "','" & fmenu.kodeuser.Text & "','" & fmenu.kodeuser.Text & "',now(),now());SELECT LAST_INSERT_ID();"
            cmmd = New OdbcCommand(sql, cnn)
            idlunaspiutang = CInt(cmmd.ExecuteScalar())
            ...
            For i As Integer = 0 To GridView1.RowCount - 1
                ...
                myCommand.CommandText = "INSERT INTO tb_pelunasan_piutang_detail(pelunasan_piutang_id, penjualan_id, pelanggan_id, tanggal_penjualan, tanggal_jatuhtempo, total_penjualan, bayar_piutang, sisa_piutang, terima_piutang, created_by, updated_by, date_created, last_updated) VALUES ('" & idlunaspiutang & "', '" & GridView1.GetRowCellValue(i, "penjualan_id") & "', '" & GridView1.GetRowCellValue(i, "pelanggan_id") & "', '" & Format(tglpenjualansimpan, "yyyy-MM-dd HH:mm:ss") & "','" & Format(tgljatuhtemposimpan, "yyyy-MM-dd HH:mm:ss") & "','" & GridView1.GetRowCellValue(i, "total_penjualan") & "','" & GridView1.GetRowCellValue(i, "bayar_piutang") & "','" & GridView1.GetRowCellValue(i, "sisa_piutang") & "','" & GridView1.GetRowCellValue(i, "terima_piutang") & "','" & fmenu.kodeuser.Text & "','" & fmenu.kodeuser.Text & "',now(),now())"
                myCommand.ExecuteNonQuery()
            Next

            myCommand.CommandText = "INSERT INTO tb_transaksi_kas (kode_kas, kode_piutang, jenis_kas, tanggal_transaksi, keterangan_kas, debet_kas, kredit_kas, created_by, updated_by, date_created, last_updated) VALUES ('" & idkas & "','" & idlunaspiutang & "', 'BAYAR', now(), 'Transaksi Pelunasan Nomor " & idlunaspiutang & "','" & 0 & "', '" & totalbayar & "', '" & fmenu.kodeuser.Text & "', '" & fmenu.kodeuser.Text & "', now(), now())"
            myCommand.ExecuteNonQuery()
            ...
            For i As Integer = 0 To GridView1.RowCount - 1
                If (bayarjual(i) + Val(GridView1.GetRowCellValue(i, "terima_piutang"))).Equals(totaljual(i)) Then
                    ...
                    myCommand.CommandText = "UPDATE tb_penjualan SET lunas_penjualan = '" & lunasstatus & "' WHERE id = '" & GridView1.GetRowCellValue(i, "penjualan_id") & "' "
                    myCommand.ExecuteNonQuery()
                Else
                    lunasstatus = 0
                End If
        ...
        If printstatus.Equals(True) Then
            ...
            If cekcetakan(txtnolunaspiutang.Text, namaform).Equals(True) Then
                ...
                If statusizincetak.Equals(True) Then
                    Call cetak_faktur()
                    Call koneksi()
                    sql = "UPDATE tb_pelunasan_piutang SET print_lunas = 1 WHERE id = '" & txtnolunaspiutang.Text & "' "
                    cmmd = New OdbcCommand(sql, cnn)
                    dr = cmmd.ExecuteReader()
    ...
    Sub perbarui(nomornota As String)
        ...
        For i As Integer = 0 To GridView1.RowCount - 1
            ...
            'IFNULL(SUM(kredit_kas), 0)
            Call koneksi()
            sql = "SELECT IFNULL(SUM(terima_piutang), 0) AS terima_piutang FROM tb_pelunasan_piutang_detail WHERE penjualan_id = '" & GridView1.GetRowCellValue(i, "penjualan_id") & "' AND pelanggan_id ='" & idpelanggan & "' AND NOT pelunasan_piutang_id ='" & nomornota & "'"
            cmmd = New OdbcCommand(sql, cnn)
            dr = cmmd.ExecuteReader
            If dr.HasRows Then
                bayarjual(i) = bayarjual(i) + Val(dr("terima_piutang"))
        ...
        Try
            myCommand.CommandText = "DELETE FROM tb_pelunasan_piutang_detail WHERE pelunasan_piutang_id = '" & nomornota & "'"
            myCommand.ExecuteNonQuery()

            'kembalikan status
            For i As Integer = 0 To tabelsementara.Rows.Count - 1
                myCommand.CommandText = "UPDATE tb_penjualan SET lunas_penjualan = '" & 0 & "' WHERE id = '" & tabelsementara.Rows(i).Item(0) & "'"
                myCommand.ExecuteNonQuery()
            Next
            '====
            ...
            For i As Integer = 0 To GridView1.RowCount - 1
                ...
                myCommand.CommandText = "INSERT INTO tb_pelunasan_piutang_detail (pelunasan_piutang_id, penjualan_id, pelanggan_id, tanggal_penjualan, tanggal_jatuhtempo, total_penjualan, bayar_piutang, sisa_piutang, terima_piutang, created_by, updated_by, date_created, last_updated) VALUES ('" & nomornota & "', '" & GridView1.GetRowCellValue(i, "penjualan_id") & "', '" & GridView1.GetRowCellValue(i, "pelanggan_id") & "', '" & Format(tglpenjualansimpan, "yyyy-MM-dd HH:mm:ss") & "','" & Format(tgljatuhtemposimpan, "yyyy-MM-dd HH:mm:ss") & "','" & GridView1.GetRowCellValue(i, "total_penjualan") & "','" & GridView1.GetRowCellValue(i, "bayar_piutang") & "','" & GridView1.GetRowCellValue(i, "sisa_piutang") & "','" & GridView1.GetRowCellValue(i, "terima_piutang") & "','" & fmenu.kodeuser.Text & "','" & fmenu.kodeuser.Text & "',now(),now())"
                myCommand.ExecuteNonQuery()
            Next

            myCommand.CommandText = "UPDATE tb_pelunasan_piutang SET user_id='" & iduser & "',tanggal_transaksi='" & Format(dtpelunasan.Value, "yyyy-MM-dd HH:mm:ss") & "', pelanggan_id='" & idpelanggan & "', kas_id='" & idkas & "', bayar_lunas='" & totalbayar & "', no_bukti='" & txtbukti.Text & "' ,keterangan_lunas='" & txtketerangan.Text & "', updated_by='" & fmenu.kodeuser.Text & "', last_updated=now()  WHERE id='" & nomornota & "'"
            myCommand.ExecuteNonQuery()

            myCommand.CommandText = "UPDATE tb_transaksi_kas SET kode_kas='" & idkas & "', tanggal_transaksi='" & Format(dtpelunasan.Value, "yyyy-MM-dd HH:mm:ss") & "', keterangan_kas='" & txtketerangan.Text & "', kredit_kas='" & totalbayar & "', updated_by='" & fmenu.kodeuser.Text & "', last_updated=now() WHERE kode_piutang='" & nomornota & "'"
            myCommand.ExecuteNonQuery()
            ...
            ElseIf btnedit.Text.Equals("Update") Then
                If cmbsales.Text IsNot "" Then
                    If cmbbayar.Text IsNot "" And iduser > 0 Then
                        If txttotalbayar.Text > 0 Then
                            If totalbayar.Equals(Val(GridView1.Columns("terima_piutang").SummaryItem.SummaryValue)) Then
                                Call perbarui(txtnolunaspiutang.Text)
                            Else
                                If totalselisih > 0 Then
                                    MsgBox("Pembayaran Lebih " & Format(totalselisih, "##,##0").ToString)
                                ElseIf totalselisih < 0 Then
    ...
    Sub caripenjualan()
        Call koneksi()
        sql = "SELECT * FROM tb_penjualan WHERE id = '" & txtkodepenjualan.Text & "' AND pelanggan_id ='" & idpelanggan & "' AND total_penjualan <> bayar_penjualan AND lunas_penjualan = 0 LIMIT 1"
        cmmd = New OdbcCommand(sql, cnn)
        dr = cmmd.ExecuteReader
        If dr.HasRows Then
            kodepenjualantbh = dr("id")
            kodepelanggantbh = dr("pelanggan_id")
            ...
            'IFNULL(SUM(kredit_kas), 0)
            sql = "SELECT IFNULL(SUM(terima_piutang), 0) AS terima_piutang FROM tb_pelunasan_piutang_detail WHERE penjualan_id = '" & txtkodepenjualan.Text & "' AND pelanggan_id ='" & idpelanggan & "'"
            cmmd = New OdbcCommand(sql, cnn)
            dr = cmmd.ExecuteReader
            If dr.HasRows Then
    ...
    Sub tambah()
        ...
        Else
            Call koneksi()
            sql = "SELECT * FROM tb_penjualan WHERE id = '" & kodepenjualantbh & "' AND pelanggan_id ='" & kodepelanggantbh & "' AND total_penjualan <> bayar_penjualan AND lunas_penjualan = 0 LIMIT 1"
            cmmd = New OdbcCommand(sql, cnn)
            dr = cmmd.ExecuteReader
            If dr.HasRows Then
                ...
                Else
                    ...
                    For i As Integer = 0 To GridView1.RowCount - 1
                        If String.Equals(GridView1.GetRowCellValue(i, "penjualan_id").ToString, kodepenjualantbh.ToUpper) Or GridView1.GetRowCellValue(i, "penjualan_id").ToString.Equals(kodepenjualantbh.ToUpper) Then
                            lokasi = i
                        End If
                    Next
